...
- Live burn-down statusline + a high-frequency `redline watch` ticker.
...
- Local analytics: `redline stats` - "did it land within budget?" (no network).
- `redline` CLI (install / uninstall / stats / watch / version); Homebrew + curl + git install.
